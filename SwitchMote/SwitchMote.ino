...
void setup(void)
{
  ...
  DEBUG("Frequency:");DEBUGln(CONFIG.frequency);
  DEBUG("SYNC_INFO: ");
...
void loop()
{
  ...
  if (radio.receiveDone())
  {
    ...
    //listen for SSR status check
    if (radio.DATALEN == 5 
    	&& radio.DATA[0]=='S' && radio.DATA[1]=='S' && radio.DATA[2]=='R' && radio.DATA[3]=='S'
    	&& radio.DATA[4]=='T')
    {
      if (radio.ACK_REQUESTED) radio.sendACK(); //send ACK sooner when a ON/OFF + ACK is requested
      
      sprintf(buff, "SSR:%d", mode[BTNINDEX_SSR]);
    ...
    if (radio.ACK_REQUESTED) //dont ACK broadcasted messages except in special circumstances (like SYNCing)
    {
      radio.sendACK();
      DEBUG(" - ACK sent");
      delay(5);
